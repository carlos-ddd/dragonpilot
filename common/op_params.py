...
class opParams:
  def __init__(self):
    ...
    self.fork_params = {
      'accel_test_allowable': Param(False, bool, 'allow the execution of accel tests!', live=True),
      'accel_test_gas_schedulable': Param(False, bool, 'allow the execution of a gas test, trigger by changing accel_test_gas_val or accel_test_target_speed_gas', live=True),
      'accel_test_brake_schedulable': Param(False, bool, 'allow the execution of a gas test, trigger by changing accel_test_brake_val or accel_test_target_speed_brake', live=True),
      'accel_test_target_speed_gas': Param(0.0, NUMBER, '[km/h] target speed for gas test at which the test will be declared done', live=True),
      'accel_test_target_speed_brake': Param(200.0, NUMBER, '[km/h] target speed for brake test at which the test will be declared done', live=True),
      'accel_test_gas_val': Param(0, NUMBER, '[raw] 227..1240 constant apply_gas for testing in a gas test', live=True),
      'accel_test_brake_val': Param(0, NUMBER, '[raw] 0..8190 (?) constant apply_brake for testing in a breake test', live=True),
      #'a_static_toggle': Param(False, bool, 'Another toggle for a feature, gotten on start up', static=True),  # this is only read once when opParams in initialized
    }
